...
const KEY_DONATION_COUNT = "numDonations";
...
const BASE_URL = "https://extralife.donordrive.com/api/";
...
function initHelper() {
    ...
    window.setTimeout(startHelper, 5000);
    if (IS_DEBUG) {
        $( "body" ).click(function() {
...
function showTestDonation() {
    testName = "John Smith"+Math.random();
    testAmount = 100.00;
...
function onGeneralInfoSuccess(res) {
    ...
    var donations = res[KEY_DONATION_COUNT];
    $("#donationCountText").text(donations)
...
function showDonationCounter(){
    log("showing trackerSlide");
    // $("#trackerSlide").show();
    $( "#trackerSlide" ).animate({
        left: "0px",
    }, 500 );
